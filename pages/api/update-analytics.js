...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!analysisType || !['general_analysis', 'redo_analysis'].includes(analysisType)) {
      return res.status(400).json({ message: "Invalid or missing analysisType parameter" });
    }
    ...
    if (userData.requests) {
      ...
      if (userData.requests[analysisType] && typeof userData.requests[analysisType] === 'object') {
